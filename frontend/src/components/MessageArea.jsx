...
setShowPicker(false)
  ...
  return (
    <div className={`lg:w-[70%] relative ${selectedUser ? "flex" : "hidden"} lg:block w-full h-full bg-slate-200 border-l-2 border-gray-600`}>
      {selectedUser && <div className='w-full h-[100vh]  flex flex-col'>
      <div className='w-full h-[100px] bg-[#1797c2] rounded-b-[30px] shadow-gray-400 shadow-lg flex  items-center px-[20px] gap-[20px]'>
        <div className='cursor-pointer' onClick={() => dispatch(setSelectedUser(null))} >
          <IoArrowBack className='w-[30px] h-[30px] text-white' /></div>
...
{selectedUser && <div className='lg:w-[70%] w-full h-[100px] fixed bottom-[20px] flex items-center justify-center   '>
   <img src={frontendImage} alt="" className='w-[80px] absolute bottom-[100px] right-[20%] rounded-lg shadow-gray-400 shadow-lg'/>
        <form className='w-[95%] lg:w-[70%] h-[60%] bg-[#1797c2] rounded-full shadow-gray-400 shadow-lg flex gap-[20px] items-center px-[20px]' onSubmit={handleSendMessage}>
         
          <div>
            <RiEmojiStickerLine className='w-[25px] h-[25px] text-white cursor-pointer' onClick={()=>setShowPicker(prev=>!prev)} />
          </div>
